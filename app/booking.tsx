...
export default function Booking() {
  ...
  const { id, name, price, city } = useLocalSearchParams();
  ...
  async function handleConfirm() {
    ...
    if (error) {
      ...
      router.push(`/payment?name=${encodeURIComponent(facilityName)}&lane=${encodeURIComponent(selectedLane)}&time=${encodeURIComponent(selectedTime)}&price=${pricePerHour}`);
    }
    setLoading(false);
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.location}>📍 {facilityCity}, AZ</Text>
